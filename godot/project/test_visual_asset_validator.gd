...
func _init() -> void:
	...
	assert(report.entries.any(func(entry): return "triangle_budget_exceeded" in entry.issues))
	assert(report.entries.any(func(entry): return "extreme_bounds_prototype" in entry.issues))
	assert(ValidatorScript.new().to_markdown(report).contains("Visual Asset Validation"))
	assert(ValidatorScript.new().write_reports(report, "user://visual_asset_validation.json", "user://visual_asset_validation.md"))
