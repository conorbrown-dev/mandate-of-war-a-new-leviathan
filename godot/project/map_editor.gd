...
func _save() -> void:
	# The editable draft is the input consumed by Load; keep native export as
	# the game-facing bundle without losing the round-trip editor workflow.
	if not model.save_to_file("user://map_editor_draft.json"):
		_message("Save failed")
		return
	_message("Map saved and native bundle exported" if model.export_native_bundle("user://map_editor_export") else "Draft saved; native export failed")
