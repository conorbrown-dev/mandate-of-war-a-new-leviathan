...
- Bootstrap: `godot/project/main.tscn` is instantiated and `_on_start_skirmish_pressed()` starts the legacy presentation slice. The explicit deterministic route is `native_skirmish.tscn`, covered by `test_native_skirmish.gd` from setup through terminal result, replay verification, and rematch. `SkirmishConfig.load_definition()` loads scenario JSON and heightmap data.
