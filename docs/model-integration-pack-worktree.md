...
python3 scripts/audit_model_integration_assets.py \
  ../mandate-of-war/downloaded_assets/Mandate_of_War_CC0_Asset_Import_Pack \
...
```bash
/home/conor/repos/mandate-of-war/Godot_v4.7.2-stable_linux.x86_64 \
...
The branch keeps this mode available through `RTS_PROTOTYPE_VISUALS` and uses
it by default for normal small skirmishes (`RTS_PROTOTYPE_VISUALS=0` restores
the procedural fallback), while
...
## Stage 16 user-provided reference replacement

The active ground, air, and logistics visual definitions now use converted
donors from `downloaded_assets/reference_models/`, generated by
`scripts/import_reference_models.py`. The conversion embeds textures, reduces
prototype geometry to the 20,000-triangle gate where needed, and writes bounds
sidecars. Stable visual IDs remain unchanged. The supplied naval directory only
contains a carrier donor; its converted scene is retained for later cleanup but
the active destroyer and carrier definitions stay on the existing lightweight
placeholders until a dedicated naval optimization pass is complete.
