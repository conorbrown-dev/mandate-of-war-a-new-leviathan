...
| Goal 05 — modding, asset pipeline, map editor | `ACTIVE` | Generated-unit development spawning, native map save/load, and single-mod dependency/version enforcement are verified: map terrain/resources/spawns/entities round-trip; a test mod's unit/mesh pair spawns with authored data; missing/incompatible `>=` dependencies reject (168 direct assertions; 2026-09-12). Batch ordering, editor surface, and required review remain. |
