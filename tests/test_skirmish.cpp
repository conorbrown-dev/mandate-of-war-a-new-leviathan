...
TEST(skirmish_logistics_orders_recover_and_resupply) {
    ...
    auto& storage = simulation.production_manager().storages().at(match.base(0));
    const auto old_energy = storage.energy_storage;
    const auto old_material = storage.metal_storage;
    ...
    verify(storage.energy_storage < old_energy,"resupply consumes owned energy");
    verify(storage.metal_storage < old_material,"resupply consumes owned material");
    boat->fuel = 0.0f;
    boat->is_stranded = true;
    storage.energy_storage = 0.0f;
    const auto fuel_before_rejected = boat->fuel;
    verify(simulation.issue_return_commands({vessel},FactionId::ELITE_PRECISION)==0,"resupply rejects insufficient energy");
    verify(boat->fuel == fuel_before_rejected && boat->is_stranded,"rejected resupply leaves vessel and stock unchanged");
