...
LogisticsResult run_logistics_scenario(int unit_count, int carrier_count, int tick_count) {
    ...
    // Spawn airborne conventional aircraft.  This intentionally avoids combat
    // components so the timed workload is recovery lookup, safe-return
    // estimation, moving carrier facilities, and intelligence—not combat.
    std::vector<rts::EntityId> aircraft_ids;
    aircraft_ids.reserve(static_cast<std::size_t>(unit_count));
    for (int i = 0; i < unit_count; ++i) {
        const float offset_x = static_cast<float>((i * 7) % 50) - 25.0f;
        const float offset_y = static_cast<float>((i * 11) % 50) - 25.0f;
        const rts::Entity aircraft = simulation.create_unit(offset_x, offset_y);
        simulation.component_manager().remove_component<rts::Weapon>(aircraft.id);
        simulation.logistics_manager().add_aircraft(aircraft.id, offset_x, offset_y, 100.0f);
        aircraft_ids.push_back(aircraft.id);
    }

    // A stationary conventional recovery point ensures the lookup has both
    // fixed and mobile compatible facilities to compare.
    const rts::Entity airbase_entity = simulation.create_unit(-250.0f, 0.0f);
    simulation.component_manager().remove_component<rts::Weapon>(airbase_entity.id);
    rts::Airbase airbase{};
    airbase.x = -250.0f;
    airbase.y = 0.0f;
    airbase.runway_capacity = 2;
    airbase.runway_usable = true;
    simulation.logistics_manager().add_airbase(airbase_entity.id, airbase);
    ...
    for (int tick = 0; tick < tick_count; ++tick) {
        // Move carrier recovery points before evaluating aircraft. The
        // benchmark deliberately measures the logistics subsystem rather
        // than unrelated prediction, combat, economy, or snapshot work.
        if (!carrier_ids.empty()) {
            float move_angle = static_cast<float>(tick) * 0.01f;
            for (size_t i = 0; i < carrier_ids.size(); ++i) {
                if (simulation.component_manager().get_component<rts::Carrier>(carrier_ids[i])) {
                    ...
                    simulation.logistics_manager().update_recovery_facility_position(
                        carrier_ids[i], new_x, new_y
                    );
                    if (auto* position = simulation.component_manager().get_component<rts::Position>(carrier_ids[i])) {
                        position->x = new_x;
                        position->y = new_y;
                    }
                }
        ...
        // Test intelligence updates against a real benchmark aircraft.
        if (tick % 10 == 0 && unit_count > 0) {
            const rts::EntityId unit_id = aircraft_ids.front();
            auto* unit_pos = simulation.component_manager().get_component<rts::Position>(unit_id);
            if (unit_pos) {
                simulation.logistics_manager().update_intelligence(unit_id, unit_pos->x, unit_pos->y,
                    static_cast<uint32_t>(tick));
            }
        }

        const auto tick_start = Clock::now();
        simulation.logistics_manager().update_all(50.0f);
        simulation.logistics_manager().batch_safe_return_check(
            aircraft_ids, simulation.navigation_for(aircraft_ids.front())
        );
...
void print_result(const LogisticsResult& result) {
    ...
    std::cout << "Logistics benchmark results\n"
