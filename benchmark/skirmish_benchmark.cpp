...
int main() {
    try {
        ...
        if(!simulation.production_manager().queue_unit(
                match.base(0), FactionId::ELITE_PRECISION, UnitType::ELITE_MAIN_BATTLE_TANK, -92.0f, 0.0f)) {
            throw std::runtime_error("Benchmark production order rejected");
        }
        std::vector<EntityId> movers;
        for(int f=0;f<2;++f) for(int i=0;i<500;++i) {
            // Keep the engagement centered in the sea-lane approach and well
            // outside command-center weapons, while retaining actual movement,
            // target acquisition, projectiles, destruction, and production.
            const float x=(f==0?-1:1)*(70.0f+float(i%50)*0.35f);
            // Both columns are on their respective landmasses and converge
            // through the authored causeway at y=0.
            const float y=-5.0f+float(i/50)*1.0f;
        ...
        for(int tick=0;tick<400;++tick) {
            ...
            if(tick<10) for(int index=tick*100;index<(tick+1)*100;++index) {
                ...
                if(simulation.issue_commands({movers[index]},static_cast<FactionId>(faction),CommandType::MOVE,
                    faction==0?8.0f:-8.0f,0)!=1) throw std::runtime_error("Benchmark movement command rejected");
            }
            const auto start=std::chrono::steady_clock::now();
            // Deliberately advance the simulation rather than the match
            // controller: a benchmark needs the full fixed sample window,
            // whereas a legitimate command-center kill may end a real match
            // mid-sample. Terminal/match-result behavior is covered by
            // test_skirmish; this is the isolated active-workload evidence.
        ...
        std::cout << "G08 workload diagnostics: initial=" << initial.entity_ids.size()
                  << " survivors=" << final.entity_ids.size() << " moving_ticks=" << moving_samples
                  << " projectile_ticks=" << projectile_samples
